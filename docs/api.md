...
| GET | `/equipamentos/etiquetas.pdf?lote=…\|ids=…&formato=` | adminOnly — folha A4 em memória. `formato`: `corte` (padrão), `pimaco6180` (A4260, 10/folha) ou `pimacoA4263` (A4263 / Avery L7163, 14/folha, 99 × 38,1 mm). **Recusa** gerar se a URL pública for local (defina `PUBLIC_BASE_URL`; `&forcar=1` ignora, só para teste) |
